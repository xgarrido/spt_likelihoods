...
class SPT3G_2018_TTTEEE_Ini_Foregrounds(HasLogger):
  ...
  def __init__( self,
    ...
    self.tSZ_template = np.zeros( self.SPT3G_windows_lmax+1)
    self.tSZ_template[np.array(ltSZ,int)] = dl_tSZ
    self.tSZ_template /= self.tSZ_template[3000] # Ensure normalisation

    # Read in kSZ template and normalise
    lkSZ, dl_kSZ = np.loadtxt( os.path.join(data_folder, kSZ_template_filename), unpack=True)
    self.kSZ_template = np.zeros( self.SPT3G_windows_lmax+1)
    self.kSZ_template[np.array(lkSZ,int)] = dl_kSZ
    self.kSZ_template /= self.kSZ_template[3000] # Ensure normalisation
  ...
  def ApplyCalibration(self, cal1, cal2, cal3, cal4, Dl_theory, Dl_foregrounds):
  ...
  def AddPoissonPower(self, pow_at_3000, Dl_theory, Dl_foregrounds):
  ...
  def AddGalacticDust(self, pow_at_80, alpha, beta, nu1, nu2, Dl_theory, Dl_foregrounds):
    ...
    Dl_galdust = Dl_galdust * DustFreqScaling(beta, T_galdust, self.nu_0_galdust, nu1) * DustFreqScaling(beta, self.T_galdust, self.nu_0_galdust, nu2)
  ...
  def AddCIBClustering(self, pow_at_3000, alpha, beta, nu1, nu2, zeta1, zeta2, Dl_theory, Dl_foregrounds):
    ...
    Dl_cib_clustering = Dl_cib_clustering * DustFreqScaling(beta, self.T_CIB, self.nu_0_CIB, nu1) * DustFreqScaling(beta, self.T_CIB, self.nu_0_CIB, nu2)
  ...
  # Cosmology scaling not supported
  def AddtSZ(self, pow_at_3000, nu1, nu2, H0, sigma_8, omb, Dl_theory, Dl_foregrounds):

    # Calculate tSZ power
    Dl_tSZ = pow_at_3000 * tSZ_template # Template
    Dl_tSZ = Dl_tSZ * tSZFrequencyScaling(nu1, self.nu_0_tSZ, T_CMB) * tSZFrequencyScaling(nu2, self.nu_0_tSZ, T_CMB) # Frequency scaling
  ...
  def AddtSZCIBCorrelation( self,
                            xi_tsz_CIB, tsz_pow_at_3000, CIB_pow_at_3000,
  ...
  # Template normalised at ell=3000
  # Cosmology scaling not supported
  def AddkSZ(self, pow_at_3000, H0, sigma_8, omb, omegam, ns, tau, Dl_theory, Dl_foregrounds):
  ...
  def ApplySuperSampleLensing(self, kappa, Dl_theory, Dl_foregrounds):
  ...
  def ApplyAberrationCorrection(self, ab_coeff, Dl_theory, Dl_foregrounds):
